...
function sassToCss() {
	return src([paths.source + '/assets/scss/style.scss'])
		.pipe(
			gulpsass({
	...
	return src([paths.source + '/*.html'])
		.pipe(
			fileinclude({
				...
				basepath: '@file',
				context: {
					assetspath: json.siteConfig.baseURL,
					css: 'style.min.css',
					lang: 'en',
					dir: 'ltr',
					urlfix: '',
...
// remove dist folder
function distclean() {
	return src(paths.destination, { allowEmpty: true }).pipe(
		clean({ force: true })
	);
}
...
// copy script folder
function copyUtils() {
	return src([paths.source + '/assets/scripts/utils.js']).pipe(
...
function runbrowser() {
	...
	// watch([paths.source + '/assets/scripts/utils.js']).on('change', () => {
	// 	copyUtils();
	// 	console.log('utils js copy');
	// });
	// watchallfolders();
